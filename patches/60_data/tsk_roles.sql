...
USING (
    SELECT 'ADMIN' AS role_id, 'Admin' AS role_name, 'Admin' AS role_group, NULL AS role_desc, 'Y' AS is_active, 99 AS order# FROM DUAL UNION ALL
    ...
    SELECT 'DEFAULT_USER' AS role_id, 'Default User' AS role_name, 'Main' AS role_group, 'Needed so user can access and use the app (create/update tasks)' AS role_desc, 'Y' AS is_active, 50 AS order# FROM DUAL UNION ALL
