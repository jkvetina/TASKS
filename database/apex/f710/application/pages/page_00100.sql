...
'        const target    = p.target.parentElement.getAttribute(''id'');',
'        const status_id = target.replace(''STATUS_'', '''');',
...
'        sorted = sorted.join('':'');',
'        //',
'        console.log(''TASK_MOVED'', task_id, status_id, sorted);',
'        //',
...
'    ) LOOP',
'        HTP.P(''<div class="TASK" draggable="true" id="TASK_'' || t.task_id || ''"><a href="'' || t.task_link || ''">'' || t.task_name || ''</a></div>'');',
'    END LOOP;',
'    --',
'    HTP.P(''</div>'');',
'END LOOP;',
