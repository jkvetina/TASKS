...
'        const status_id     = target.replace(''STATUS_'', '''').split(''__'')[0];',
'        const swimlane_id   = target.replace(''STATUS_'', '''').split(''__'')[1];',
...
'',
'    -- swimlanes per user',
'    HTP.P(''<div class="SWIMLANE" id="SWIMLANE_'' || w.swimlane_id || ''">'');',
'    HTP.P(''<div class="COLUMNS">'');',
'    HTP.P(''<h3>'' || NULLIF(''@'' || w.swimlane_name, ''@-'') || ''</h3>'');',
'    --',
'    FOR s IN (',
'        SELECT s.*',
'        FROM tsk_task_statuses s',
...
'        ORDER BY s.order#',
'    ) LOOP',
'        HTP.P(''<div class="TARGET" id="STATUS_'' || s.status_id || ''__'' || w.swimlane_id || ''">'');',
'        --',
'        FOR t IN (',
'            SELECT',
'                t.task_id,',
